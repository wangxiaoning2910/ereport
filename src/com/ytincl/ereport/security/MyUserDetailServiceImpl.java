package com.ytincl.ereport.security;
...
//import com.ytincl.ereport.util.PropertiesUtils;
...
//		System.err.println("-----------MyUserDetailServiceImpl loadUserByUsername ----------- ");
		...
		//取得用户的权限
		Account users =accountDao.querySingleAccount(username);
		if  (users==null)  
	...
	private Set<GrantedAuthority> obtionGrantedAuthorities(Account account) {
		...
		if("root".equals(account.getAccountName())){
			menus = resourcesDao.queryAll(new Resources());
		}else{
			menus = resourcesDao.findAccountResourcess(String.valueOf(account.getId()));
		}
